...
## Test Checklist
- [x] Run `go test ./...`.
- [ ] Run issue flow with `LETSENCRYPT_ENV=staging`.
- [x] Verify cert/key files exist.
- [x] Verify expiry in SQLite.
- [x] Verify failure behavior with missing Cloudflare token.

## Outcome
Implemented the ACME issue workflow in code: added `lego` + Cloudflare DNS-01 integration, ACME account persistence under `APP_ACCOUNT_DIR`, certificate/key file persistence under `APP_CERT_DIR`, expiry parsing/storage, issue job logging, and the `/certificates/:id/issue` web action. The UI now keeps issue and Kong sync as separate operator actions for clarity. Live Let's Encrypt verification is still pending.
...
- Live staging issue flow has not been run yet in an environment with valid Cloudflare credentials.
